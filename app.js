...
var BrushSelector, LinkPreview, Mousetrap, OrbitPreview, Pause, PauseButton, SVG, Undo, brush_buttons, d3, line, links, links_group, main_events, orbits, orbits_group, pause_button, speed, svg, update;
...
brush_buttons.on('click', function(button) {
  return BrushSelector.events.change_brush(button.name);
});
...
svg.on('mousemove.main', function() {
  return main_events.mouse_position(d3.mouse(this));
});

main_events.on('save.main', function(preview) {
  console.log('saving:', preview);
  if (preview.type === 'orbit') {
    return orbits.push(preview);
  } else if (preview.type === 'link') {
    return links.push(preview);
  }
});

Undo.events.on("undo.main", function(item) {
  if (item.type === 'orbit') {
    return orbits.pop();
...
Undo.events.on("redo.main", function(item) {
  if (item.type === 'orbit') {
    return orbits.push(item);
  } else if (item.type === 'link') {
...
update = function(t) {
  var cos, g, orbit_line, orbiters, paths, selection, sin;
  main_events.time(t);
  cos = Math.cos;
  sin = Math.sin;
  orbits = orbits.map(function(d) {
    d.orbiter = {
  ...
  selection = orbits_group.selectAll('g').data(orbits);
  selection.exit().remove();
  g = selection.enter().append('g').attr('transform', function(d) {
    return "translate(" + d.x + "," + d.y + ")";
  });
  g.append('circle').attr('cx', 0).attr('cy', 0).attr('r', function(d) {
    return d.r;
  });
  g.append('circle').attr('class', 'orbiter').attr('cx', 0).attr('cy', 0).attr('r', 3);
  orbiters = selection.select('.orbiter').attr('cx', function(d) {
    return d.orbiter.x;
  }).attr('cy', function(d) {
    return d.orbiter.y;
  });
  orbit_line = function(d) {
    return line([d.start.orbiter.absolute, d.end.orbiter.absolute]);
  };
  paths = links_group.selectAll('path').data(links).attr('d', orbit_line);
  paths.enter().append('path').attr('stroke', 'black').attr('shape-rendering', 'optimizeSpeed').attr('d', orbit_line);
...
},{"./brush-selector.coffee":4,"./link-preview.coffee":5,"./mousetrap.js":6,"./orbit-preview.coffee":7,"./pause-button.coffee":8,"./pause.coffee":9,"./resizable-svg.coffee":10,"./undo.coffee":11,"d3":2}],4:[function(require,module,exports){
var Mousetrap, activate_button, bind, d3, dispatcher, state;
...
activate_button = function(buttons, name) {
  ...
  return buttons.classed('pure-button-active', function(d) {
    return d.active === true;
  });
};
...
bind = function(selector) {
  var buttons;
  buttons = d3.select(selector).selectAll('a.brush').data(state);
  dispatcher.on('change_brush.activate', function(brush_name) {
    return activate_button(buttons, brush_name);
  });
  return buttons;
};
...
module.exports = {
  bind: bind,
  events: dispatcher,
  select_orbit_brush: function() {
    return dispatcher.change_brush('orbit');
  },
  select_link_brush: function() {
    return dispatcher.change_brush('link');
  },
  _activate_button: activate_button
};


},{"./mousetrap.js":6,"d3":2}],5:[function(require,module,exports){
var bind, closest_orbiter, current_brush, d3, measure_distance_magnitude, mouse_position, preview, render, setup, time;

d3 = require('d3');

measure_distance_magnitude = function(point1, point2) {
  var pow, r2;
  pow = Math.pow;
...
render = function(svg, preview) {
  ...
  if (preview === null) {
    return svg.select('#link-brush').style('display', 'none');
  } else {
    return svg.select('#link-brush').datum(preview).style('display', '').style('stroke', 'gray').style('stroke-width', 3).attr('x1', function(d) {
      return d.start.orbiter.absolute.x;
    }).attr('y1', function(d) {
      return d.start.orbiter.absolute.y;
    }).attr('x2', function(d) {
      return d.end.orbiter.absolute.x;
    }).attr('y2', function(d) {
      return d.end.orbiter.absolute.y;
    });
  }
};

setup = function(svg, main_events) {
  var select_closest_orbiter;
  select_closest_orbiter = function() {
    var closest_distance, orbiters, orbits;
    if (mouse_position === null) {
      return null;
    }
    orbits = [];
    orbiters = svg.selectAll('.orbiter');
    orbiters.each(function(d) {
      d.orbiter.distanceToMouseSquared = measure_distance_magnitude(mouse_position, [d.orbiter.absolute.x, d.orbiter.absolute.y]);
      return orbits.push(d);
    });
    closest_distance = d3.min(orbits, function(d) {
      return d.orbiter.distanceToMouseSquared;
    });
    closest_orbiter = orbiters.filter(function(d, i) {
  ...
  main_events.on('mouse_position.link-preview', function(new_mouse_position) {
    return mouse_position = new_mouse_position;
  });
  main_events.on('time.link-preview', function(new_time) {
    time = new_time;
    if (current_brush === 'link') {
      closest_orbiter = select_closest_orbiter(svg, preview);
      return render(svg, preview);
    }
  });
  svg.on('mousedown.preview', function() {
    if (d3.event.which !== 1) {
      return;
    }
    if (!closest_orbiter) {
    ...
    return console.log('link start', preview);
  });
  svg.on('mousemove.preview', function() {
    if (preview === null) {
      return;
    }
    return preview.end = closest_orbiter.node().__data__;
  });
  return svg.on('mouseup.preview', function() {
    if (preview === null) {
      return;
    }
...
bind = function(svg, brush_change_events, main_events) {
  return brush_change_events.on('change_brush.link-preview', function(new_brush_name) {
...
},{"d3":2}],6:[function(require,module,exports){
/*global define:false */
/**
...
var bind, d3, measure_distance, render, setup;

d3 = require('d3');

measure_distance = function(point1, point2) {
  var pow, r2;
  pow = Math.pow;
...
render = function(svg, preview) {
  ...
  return svg.select('#circle-brush').datum(preview).style('display', '').style('fill', 'none').style('stroke', 'gray').style('stroke-width', 1).attr('r', function(d) {
    return d.r;
  }).attr('cx', function(d) {
    return d.x;
  }).attr('cy', function(d) {
...
setup = function(svg, main_events) {
  ...
  svg.on('mousemove.preview', function(x, y) {
    var _ref;
    if (preview === null) {
      return;
    }
    _ref = d3.mouse(this), x = _ref[0], y = _ref[1];
    preview.r = measure_distance(preview, {
      x: x,
      y: y
    });
    return render(svg, preview);
  });
  return svg.on('mouseup', function(x, y) {
    var r, _ref;
    if (preview === null) {
      return;
    }
    _ref = d3.mouse(this), x = _ref[0], y = _ref[1];
    r = measure_distance(preview, {
      x: x,
      y: y
    });
    preview.r = r || 1;
...
bind = function(svg, brush_change_events, main_events) {
  brush_change_events.on('change_brush.orbit-preview', function(new_brush_name) {
...
},{"d3":2}],8:[function(require,module,exports){
var d3;

d3 = require('d3');

module.exports = {
  bind: function(selector, pause_events) {
    var button, label;
    button = d3.select(selector);
    label = button.select('.label');
    button.on('mouseover', function() {
      return button.classed('pure-button-active', true);
    });
    button.on('mouseout', function() {
      return button.classed('pure-button-active', false);
    });
    pause_events.on('pause.button', function() {
      return label.text(label.text().replace('pause', 'unpause'));
    });
    pause_events.on('unpause.button', function() {
      return label.text(label.text().replace('unpause', 'pause'));
    });
    return button;
  }
};
...
var d3, dispatcher, pause_start, paused, pauses, seconds, total_pause_elapsed;

d3 = require('d3');

dispatcher = d3.dispatch("pause", "unpause");

seconds = 0;

window.paused = paused = false;
...
dispatcher.on("pause", function(seconds) {
  console.log('pause start', seconds);
  pauses.push([seconds]);
  pause_start = seconds;
  return d3.timer(function(pause_elapsed) {
    dispatcher.on("unpause", function(seconds) {
      console.log('pause end', seconds);
      pauses[pauses.length - 1].push(seconds);
      return total_pause_elapsed += seconds - pause_start;
    });
    if (paused) {
      return false;
    } else {
...
init = function() {
  var svg;
  svg = d3.select('svg').attr('height', window.innerHeight).attr('width', window.innerWidth).attr('viewBox', "0 0 " + window.innerWidth + " " + window.innerHeight).attr('preserveAspectRatio', 'xMinYMin');
  d3.select(window).on('resize', function() {
    return svg.attr('height', window.innerHeight).attr('width', window.innerWidth);
  });
  return svg;
};
...
},{"d3":2}],11:[function(require,module,exports){
var Mousetrap, added, d3, dispatcher, redo;
...
module.exports = {
  init: function(input) {
    return input.on("save.undo", function(item) {
      added.push(item);
  ...
  redo: function() {
    ...
    if (item !== void 0) {
      added.push(item);
      return dispatcher.redo(item);
    }
  }
};


},{"./mousetrap.js":6,"d3":2}]},{},[3])
//# sourceMappingURL=data:application/json;base64,
